...
namespace Netcode.Transports.Facepunch
{
    ...
    public class FacepunchTransport : NetworkTransport, IConnectionManager, ISocketManager
    {
        ...
        // Holler Horror patch: track whether WE initialized Steam, so Shutdown()
        // never tears down a SteamClient owned by the game (SteamSessionManager).
        ...
        public override void Initialize(NetworkManager networkManager = null)
        {
            ...
            if (SteamClient.IsValid)
                return; // game already initialized Steam; don't double-init

            try
            {
                SteamClient.Init(steamAppId, false);
                ownsSteamClient = true;
            }
            catch (Exception e)
            {
